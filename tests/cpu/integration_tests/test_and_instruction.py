...
from cpu.constants import OP_R_TYPE, FUNC3_ALU_AND
...
async def test_and_instruction(dut):
    ...
    for rs1_value, rs2_value, expected_result in tests:
        instruction = gen_r_type_instruction(OP_R_TYPE, rd, FUNC3_ALU_AND, rs1, rs2, 0)
        ...
        assert dut.cpu.reg_file.Registers[rd].value.integer == expected_result, f"AND instruction failed: Rd value is {dut.cpu.reg_file.Registers[rd].value.integer:#010x}, expected {expected_result:#010x}"
