import cocotb
from cocotb.triggers import Timer
from cpu.utils import (
    gen_i_type_instruction,
)
from cpu.constants import (
    OP_I_TYPE_LOAD,
    FUNC3_LOAD_LHU
)

wait_ns = 1

@cocotb.test()
async def test_lhu_instruction(dut):
    """Test lhu instruction (unsigned halfword load)"""
    start_address = 96
    rd = 9
    rs1 = 10
    rs1_value = 400
    mem_value = 0xBEEF
    offset = 16
    mem_address = rs1_value + offset
    lhu_instruction = gen_i_type_instruction(OP_I_TYPE_LOAD, rd, FUNC3_LOAD_LHU, rs1, offset)
    dut.cpu.r_PC.value = start_address
    dut.cpu.instruction_memory.Memory_Array[start_address>>2].value = lhu_instruction
    dut.cpu.reg_file.Registers[rs1].value = rs1_value
    dut.cpu.mem.Memory_Array[mem_address] = mem_value & 0xFF
    dut.cpu.mem.Memory_Array[mem_address + 1] = (mem_value >> 8) & 0xFF
    dut.cpu.i_Clock.value = 0
    await Timer(wait_ns, units="ns")
    dut.cpu.i_Clock.value = 1
    await Timer(wait_ns, units="ns")
    expected_value = mem_value  # Should be zero-extended
    assert dut.cpu.reg_file.Registers[rd].value.integer == expected_value, f"LHU instruction failed: Rd value is {dut.cpu.reg_file.Registers[rd].value.integer:#010x}, expected {expected_value:#010x}"
