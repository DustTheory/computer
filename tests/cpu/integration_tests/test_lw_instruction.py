...
async def test_lw_instruction(dut):
    ...
    lw_instruction = gen_i_type_instruction(OP_I_TYPE_LOAD, rd, FUNC3_LOAD_LW, rs1, offset)
    dut.cpu.r_PC.value = start_address
    dut.cpu.instruction_memory.Memory_Array[start_address>>2].value = lw_instruction
    dut.cpu.reg_file.Registers[rs1].value = rs1_value
    # Store word in little-endian order
    dut.cpu.mem.Memory_Array[mem_address] = mem_value & 0xFF
    dut.cpu.mem.Memory_Array[mem_address + 1] = (mem_value >> 8) & 0xFF
    dut.cpu.mem.Memory_Array[mem_address + 2] = (mem_value >> 16) & 0xFF
    dut.cpu.mem.Memory_Array[mem_address + 3] = (mem_value >> 24) & 0xFF
