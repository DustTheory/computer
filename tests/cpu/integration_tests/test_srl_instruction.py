...
from cpu.utils import gen_r_type_instruction, send_write_pc_command, write_word_to_mem, wait_for_pipeline_flush, send_unhalt_command
...
async def test_srl_instruction(dut):
    ...
    srl_instruction = gen_r_type_instruction(rd, FUNC3_ALU_SRL_SRA, rs1, rs2, 0)
    ...
    for rs1_value, rs2_value, expected_result in tests:
        ...
        write_word_to_mem(dut.instruction_ram.mem, start_address, srl_instruction)
        ...
        assert actual == expected_result, (
            f"SRL failed: rs1={rs1_value:#010x} rs2={rs2_value:#010x} -> rd={actual:#010x} expected={expected_result:#010x}"
        )
