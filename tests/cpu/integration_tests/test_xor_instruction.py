...
from cpu.constants import FUNC3_ALU_XOR, PIPELINE_CYCLES, ROM_BOUNDARY_ADDR
...
async def test_xor_instruction(dut):
    ...
    for rs1_value, rs2_value, expected_result in tests:
        ...
        assert actual == expected_result, f"XOR failed: rs1={rs1_value:#010x} rs2={rs2_value:#010x} -> rd={actual:#010x} expected={expected_result:#010x}"
